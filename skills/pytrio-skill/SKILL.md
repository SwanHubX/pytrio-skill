---
name: pytrio-skill
description: 使用 PyTRIO/TRIO 编写、调试或解释远程大模型训练与推理代码。用户提到 pytrio、TRIO、PyTRIO SDK、ServiceClient、TrainingClient、SamplingClient、LoRA 训练、SFT、异步 SFT、RL、GRPO、OPD、Search-R1、Agentic RL、OPSD、On-Policy Self-Distillation、DPO、custom loss、HuggingFace datasets、TRIO OpenAI 兼容 API、SwanLab 训练记录、权重保存、checkpoint 或远程 LLM 后训练时使用。
---
...
2. 判断任务类型：SFT、GRPO、OPD、Search-R1、OPSD、DPO/custom loss、推理/保存权重，或 API 调试。
...
| 编写 Search-R1 / Agentic RL / 多轮搜索工具训练 | `references/search-r1.md`；完整项目读取其中的官方源码目录 |
| 编写 OPSD / On-Policy Self-Distillation / privileged teacher | `references/opsd.md`；完整项目读取其中的官方源码目录 |
...
- 训练脚本建议默认接入 SwanLab。SFT 记录 loss；GRPO 记录 reward、degenerate group 比例和 trainer metrics；OPD/OPSD 记录 reverse KL、completion token 数和 trainer metrics；Search-R1 额外记录 format、search success/error、turns 和 search calls；DPO 记录 loss、accuracy、margin、chosen/rejected reward。
