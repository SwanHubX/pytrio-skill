...
total_weights = np.concatenate([
    np.array(ex.loss_fn_inputs["weights"].tolist() if hasattr(ex.loss_fn_inputs["weights"], "tolist") else ex.loss_fn_inputs["weights"])
    for ex in processed_examples
]).sum()
print(f"Loss per token: {loss_sum / total_weights:.4f}")
